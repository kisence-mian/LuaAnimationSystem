...
--=================================================>> 封装函数 <<=============================================== START
	...
	--新UI统一使用的进入动画
		...
		--右耳朵
		--动画完成回调
		function newUIIntoAnim_AnimSystem(go_mainPanel,go_leftEar,go_RightEar,f_endCallBack)

			if go_mainPanel == nil then --如果主面板为空，出于容错考虑直接调用回调函数并返回

				if f_endCallBack ~= nil then
					f_endCallBack();
				end
			...
			--第一段动画完成调用
			local function inToAnim_2(t_parameters)

				if t_parameters.go_leftEar ~= nil then

					t_parameters.go_leftEar:SetActive(true);

					local v3_endPos   = getComponent(t_parameters.go_leftEar,"RectTransform").anchoredPosition3D;
					local v3_startPos = Vector3.New(v3_endPos.x - 100,v3_endPos.y,v3_endPos.z);

					AnimTo_AnimSystem(t_parameters.go_leftEar,c_anchoredPosition,v3_startPos,v3_endPos,200,c_interArithmetic_outQuad);
				end

				if t_parameters.go_RightEar ~= nil then

					t_parameters.go_RightEar:SetActive(true);

					local v3_endPos   = getComponent(t_parameters.go_RightEar,"RectTransform").anchoredPosition3D;
					local v3_startPos = Vector3.New(v3_endPos.x + 100,v3_endPos.y,v3_endPos.z);

					AnimTo_AnimSystem(t_parameters.go_RightEar,c_anchoredPosition,v3_startPos,v3_endPos,200,c_interArithmetic_outQuad,t_parameters.f_endCallBack);
				else

					if t_parameters.f_endCallBack ~= nil then
						t_parameters.f_endCallBack();
					end
				end
			...
			t_parameters.go_RightEar   = go_RightEar;
			t_parameters.f_endCallBack = f_endCallBack;

			if go_leftEar ~= nil then

				if go_leftEar.activeSelf == true then
					go_leftEar:SetActive(false);
					t_parameters.go_leftEar    = go_leftEar;
				end
			end

			if go_RightEar ~= nil then
				go_RightEar:SetActive(false);
			end


			-- AnimTo_AnimSystem(go_mainPanel,c_UGUI_alpha,0,1,500,c_interArithmetic_Lerp);
			AnimTo_AnimSystem(go_mainPanel,c_anchoredPosition,Vector3.New(0,1000,0),Vector3.New(0,0,0),500,c_interArithmetic_NewUIAnim,inToAnim_2,t_parameters);
		end
	...
	--新UI统一使用的退出动画
		...
		--右耳朵
		--动画完成回调
		function newUIExitAnim(go_mainPanel,go_leftEar,go_RightEar,f_endCallBack)
			if go_mainPanel == nil then --如果主面板为空，出于容错考虑直接调用回调函数并返回
			...
			--第一段动画完成调用
			local function exitAnim_2(t_parameters)

				AnimTo_AnimSystem(t_parameters.go_mainPanel,c_anchoredPosition,Vector3.New(0,00,0),Vector3.New(0,1000,0),200,c_interArithmetic_Lerp,
					function ()
						t_parameters.f_endCallBack()

						if t_parameters.go_leftEar ~= nil then
							getComponent(t_parameters.go_leftEar ,"RectTransform").anchoredPosition3D  = t_parameters.v3_startPos2;
						end

						getComponent(t_parameters.go_RightEar,"RectTransform").anchoredPosition3D = t_parameters.v3_startPos;

					end );
				-- AnimTo_AnimSystem(go_mainPanel,c_UGUI_alpha,1,0,200,c_interArithmetic_Lerp);
				
			end

			local t_parameters = {};
			t_parameters.go_mainPanel  = go_mainPanel;
			t_parameters.go_leftEar   = go_leftEar;
			t_parameters.go_RightEar   = go_RightEar;
			...
			if go_leftEar ~= nil then
				...
				if go_leftEar.activeSelf == true then
					...
					local v3_startPos = getComponent(go_leftEar,"RectTransform").anchoredPosition3D;
					local v3_endPos   = Vector3.New(v3_startPos.x - 100,v3_startPos.y,v3_startPos.z);

					t_parameters.v3_startPos2 = v3_startPos;

					AnimTo_AnimSystem(go_leftEar,c_anchoredPosition,v3_startPos,v3_endPos,200,c_interArithmetic_Lerp);
				end
			end

			if go_RightEar ~= nil then

				local v3_startPos   = getComponent(go_RightEar,"RectTransform").anchoredPosition3D;
				local v3_endPos     = Vector3.New(v3_startPos.x + 100,v3_startPos.y,v3_startPos.z);

				t_parameters.v3_startPos   = v3_startPos;

				AnimTo_AnimSystem(go_RightEar,c_anchoredPosition,v3_startPos,v3_endPos,200,c_interArithmetic_Lerp,exitAnim_2,t_parameters);
			else

				exitAnim_2(t_parameters);
			end
